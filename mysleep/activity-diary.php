...
if ($row['type'] != 'student') {
    log_error("ActivityDiary: bad user type.");
    header("Location: main-page");
    exit;
}
...
$config = array('lesson_num' => $lessonNum, 'activity_title' => 'Activity Diary');
$query = $_POST['query'];
?>
          ...
          <?php
           ...
           ?>
			...
			<div class="col-md-offset-3 col-md-6" style="padding-top: 1em;">
			    <h4 class="description"> Activity Diary for
				<?php
				include 'connectdb.php';
				$result = mysql_query("SELECT * FROM activity_diary_data_table where diaryId='$diaryEntryId'");
				$row = mysql_fetch_array($result);
				mysql_close($con);
				// echo $row['diaryDate'];
				// echo date(' l', strtotime($row['diaryDate']));
				echo date('l, F d, Y', strtotime($row['diaryDate']));
				?>
			    </h4>
			</div>
		    </div>

		    <form id="activityForm" action="activity-diary-done" method="post">
