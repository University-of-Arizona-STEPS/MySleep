...
if($userType == 'student'){
   ...
   $resultsUserDiaries = mysql_query("SELECT userId, diaryDate FROM practice_diary_data_table WHERE userId='$userId' AND diaryDate='$diaryId' AND diaryGrade='$currentGrade'");
if (mysql_num_rows($resultsUserDiaries) <= 0) {
    mysql_query("INSERT INTO practice_diary_data_table(userId, diaryGrade, diaryDate) VALUES ('$userId', '$currentGrade', '$diaryId')");
}
mysql_close($con);
?>
...
if($currentGrade == 4){
	...
	$status = mysql_query("UPDATE practice_diary_data_table SET " .
...
if ($sleepTimeHour == "")
    $status = mysql_query("UPDATE practice_diary_data_table SET hourSlept=NULL WHERE diaryDate='$diaryId' and userId='$userId' and diaryGrade='$currentGrade' ");
