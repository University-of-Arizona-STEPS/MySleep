  ...
  <body>
        ...
        <div class="main main-raised">
                ...
                $resultRow = explode(',',ltrim($_POST['records'], ','));
        		    ...
        		    /*				Save to MySQL        */
        		    /*-----------------------------------------------*/
                $update_query = "UPDATE fourthGradeLessonOnePreInterview SET ";
                $columns = Array('score' => 'score = CASE ', 'comment' => 'comment = CASE ');
                foreach ($resultRow as $index => $id) {
                  $columns['score'] .= "WHEN userId='" .$id. "' THEN '" . $score[$index] . "' ";
                  $columns['comment'] .= "WHEN userId='" . $id . "' THEN '" . $comment[$index] . "' ";
                }
                foreach($columns as $column_name => $query_part){
                  $columns[$column_name] .= " ELSE '$column_name' END ";
                }
                $where = " WHERE userId='" . implode("' OR userId='", $resultRow) . "'";
    		    ...
    		    <div class="row top">
                    ...
                    <a class="btn btn-gradbg btn-large btn-block"  name="Continue" href="adult-pre-interview-non-student-review<?php echo "?".$query; ?>">Continue</a>
          			</div>
    		    </div>
    		</div>
    	    </div>
