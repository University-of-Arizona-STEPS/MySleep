                ...
                <div class="container">
                  ...
                  navigationLinkReview($config,$userType);
                   ...
                   $tableBorder = 'borderless';
                     ...
                     $tableBorder = '';
                     ?>
                   <form action="do-animal-sleep-review-done" method="post">
                   <?php } ?>
                   <div class="row">
                     <div class="col-xs-10 col-md-10">
              				<table id="answers-table"  class="col-md-offset-1 col-md-10 col-sm-offset-1 col-sm-10 table <?php echo $tableBorder; ?>">
              				    <thead>
                    					<tr>
                    					    <?php if($showToClass == "1"){ ?>
                    						        <th>Bottom Line</th>
                    					    <?php }else{ ?>
                    						        <th>Name</th><th>Bottom Line</th>
                    					    <?php
                              ...
                              <?php if($showToClass == "0"){
                                ...
                                if(($userType == 'teacher') || ($userType == 'parent')) {
                                    if($userType == 'teacher') {
                                        $classId = $_SESSION['classId'];
                                        if ($config) {
                                          $result = mysql_query("SELECT * FROM bottomLine WHERE classId='$classId' and submit='1'");

                                        } else {
                                            $resultLink = getUserIdsInClass($classId);
                                            $students = join("','",$resultLink);
                                            $result = mysql_query("SELECT * FROM bottomLine WHERE userId IN ('$students') AND submit IS NOT NULL");
                                        }
                                    } else {
        	 ...
        	 // console.log(response.row);
                   ...
                   if (i%2==0) {
                     ...
                     color = 'bubble-bb';
        			     ...
        			     prerow = "<tr><td><div class='speech-bubble "+color+"'><div class='bubbletext'>" + response.content[i] + "</div></div></td><tr>";
        			     $("#answers-table tbody").append(prerow);
        		     }
        		   }
