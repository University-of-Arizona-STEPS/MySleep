...
$diaryEntryId = $_POST['diaryEntryId'];     // Use _POST (not _SESSION) because of the form in previous page
...
//debugToConsole('diary', $diaryComputation);
mysql_close($con);
...
<html>
	...
	<div class="wrapper" >
       ...
       ?>
			...
			<?php if($currentGrade == 4){ ?>
			    <li><a href="#" class = "exit" data-location="fourth-grade-lesson-menu?lesson=1">Lessons One</a></li>
			    <li><a href="#" class = "exit" data-location="diary-menu">Activity One: Gathering and Recording Data</a></li>
			<?php }else{ ?>
			    <li><a href="#" class = "exit" data-location="fifth-grade-lesson-menu?lesson=2">Lessons Two</a></li>
			    <li><a href="#" class = "exit" data-location="diary-menu">Activity One: Gathering and Recording Data</a></li>
			<?php } ?>
        ...
        <?php } ?>
			...
			<div class="col-md-offset-3 col-md-6" style="padding-top: 1em;">
			    <h4 class="description"> Sleep Diary for
				<?php
				include 'connectdb.php';
				$result = mysql_query("SELECT * FROM diary_data_table where diaryId='$diaryEntryId'");
				$row = mysql_fetch_array($result);
				mysql_close($con);
				//echo $row['diaryDate'];
				echo date('l, F j, Y', strtotime($row['diaryDate']));
				?>
			    </h4> <br>

			    <h4 class="description">Please complete this each morning when you wake up. The information you provide about bedtime/sleep is about the previous night's sleep. If you forget to complete it upon awakening, please complete as soon as possible and select the correct date.</h4><br>
		    ...
		    <form id="diaryForm" name="diaryForm" action="sleep-diary-done" method="post">
			...
			<div class="prompt_entry">
				...
				<button type="button" class="btn btn-default btn-sm" data-toggle="popover" data-placement="right" title="" data-content="Attempted to fall asleep means you were in bed with the intent to go to sleep---this may mean turning lights off, saying goodnight to someone, or closing your eyes.">Info</button>
			    </div>

			</div>
			<div class="prompt_entry">
			    <label class="prompt">Number of minutes it took to fall asleep:</label>
				<div class="entry">
			...
			<div class="prompt_entry">
			    <label class="prompt">Total minutes my sleep was interrupted for across all awakenings:</label>
			    <div class="entry">
			...
			<div class="prompt_entry">
				...
				<button type="button" class="btn btn-default btn-sm" data-toggle="popover" data-placement="right" title="" data-content="Final awakening(did not fall back asleep)">Info</button>
			    </div>
			...
			<div class="col-md-offset-4 col-md-4 col-sm-offset-4 col-sm-4">
			    <a class="btn btn-success btn-large btn-block" id ="submit-form">Submit</a>
			</div>
		    </div>
	        </div>
	    </div>
	...
	<script>
	 ...
	 $("#submit-form").click(function(){
		 ...
		 if(!$('[name="sleepQuality"]').is(':checked') || !$('[name="sleepCompare"]').is(':checked') || !$('[name="wokeup_type"]').is(':checked') ){
		     $("#alertMessage").text("Please fill in all required fields.");
		     $("#alert").modal('show');
		     return false;
		 }
